    ...
    <form action="{{Route('coupon.store')}}" method="POST" class="mt-4 mb-4">
        ...
        <div class="row">
            <div class="input-group mb-3 col">
                ...
                <input type="text" class="form-control" value="{{$generatedCode}}" disabled>
                <input type="hidden" class="form-control" id="code" name="code" value="{{$generatedCode}}">
            </div>
            <div class="input-group mb-3 col">
                <span for="quantity" class="input-group-text">Number of Coupons</span>
        ...
        <div class="input-group mb-3">
            ...
            <select class="form-select" name="applies">
                <option value="All Products" {{ old('applies') === 'All Products' ? 'selected' : '' }}>All Products</option>
                @foreach ($products as $product)
                    <option value="{{$product->id}}" {{ old('applies') === $product->id ? 'selected' : '' }}>{{$product->name}}</option>
                @endforeach
            </select>
        </div>
