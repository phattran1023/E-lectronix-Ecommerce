            ...
            <div class="col-md-4 col-lg-3">
                <div class="card shadow-sm border-light mb-4">
                    <a href="{{url('collections/'.$product->categoryGetName->slug.'/'.$product->slug)}}"
                        ...
                        <img src="{{ asset($product->productImages->first()->image) }}" class="card-img-top" alt="image"
                            height="200px"> 
                    </a>
                    <div class="card-body">
                        <a class="card-title" href="{{url('collections/'.$product->categoryGetName->slug.'/'.$product->slug)}}">
                            <h5 class="font-weight-normal" style="text-decoration: none; overflow:hidden; height:50px">
                                {{ $product->name }}
                            </h5>
                        </a>
                        <div class="post-meta"><span class="small lh-120"><i class="fas fa-map-marker-alt mr-2"></i>{{
                                $product->brand }}</span>
                        </div>
                        ...
                        <div class="d-flex justify-content-between">
                            <div class="col pl-0"><span class="text-muted font-small d-block mb-2">Price</span> <span
                                    class="h5 text-dark font-weight-bold">${{ $product->original_price }}</span></div>
                            <div class="col pr-0"><span class="text-muted font-small d-block mb-2">Sale</span> <span
                                    class="h5 text-dark font-weight-bold">${{ $product->selling_price }}</span></div>
                        </div>
                    </div>
                </div>
            </div>
